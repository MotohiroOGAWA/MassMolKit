...
def get_paths_for_matched_peaks(
    ...
    all_formulas_with_node_id = fragment_tree.get_all_formulas_with_node_id(adduct_type, supported_adduct_types=supported_adduct_types)
        ...
        if info['n_matches'] > 0:
            for formula_str, mass_error in zip(info['matched_formulas'], info['mass_errors']):
                ...
                for node_id in formula_with_node_id[1]:
                    paths = _collect_path_to_root(fragment_tree, node_id)
                    return paths

def get_paths_to_root(fragment_tree: FragmentTree, node_id: int) -> List[str]:
    path = _collect_path_to_root(fragment_tree, node_id)
    return path
